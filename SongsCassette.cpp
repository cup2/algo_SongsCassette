...
void generateSequence(int* sequence, const int& S, const int& A, const int& B, const int& M, const int& n) {
	sequence[0] = S;
	for (int i = 1; i < n; ++i) {
...
void sortToMgoodSongs(int* sequence, int first, int last,  const int& m) {
...
int findSumToM(int* sequence, const int&  m) {
	int sum = 0;
	for (int i = 0; i < m; i++) {
		sum += sequence[i];
	}
